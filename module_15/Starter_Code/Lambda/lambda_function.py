...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        investment = parse_int(investment_amount)

        is_Valid = None

        if age_int < 0:
            is_Valid = build_validation_result(False, "age", "Please enter a valid age.")
        elif age_int >= 65:
            is_Valid = build_validation_result(False, "age", "Sorry, you are unfortunately too old for this service. You may enter any age below 65.")
        
        if investment < 5000:
            is_Valid = build_validation_result(False, "investmentAmount", "Sorry, the minimum investment accepted is $5000. Could you provide an investment greater than or equal to $5000?")
        
        if(age_int > 0 and age_int < 65 and investment >= 5000):
            is_Valid = build_validation_result(True, None, None)

        if not is_Valid["isValid"]:
            get_slots(intent_request)[is_Valid["violatedSlot"]] = None # delete invalid slot
            return elicit_slot(intent_request["sessionAttributes"],
                               intent_request["currentIntent"]["name"],
                               get_slots(intent_request),
                               is_Valid["violatedSlot"],
                               is_Valid["message"]) # ask for new valid response
    ...
    recommendation = risk_levels[risk_level.lower()]
    return close(intent_request["sessionAttributes"], "Fulfilled",
                 {
                     "contentType": "PlainText",
                     "content": "Hello {}, thank you for using this service. Based on your risk level, I recommend to build your portfolio with {}".format(first_name, recommendation)
                 })
